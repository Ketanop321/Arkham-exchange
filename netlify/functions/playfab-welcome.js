...
export async function handler(event, context) {
  ...
  try {
    ...
    // If using local session, return welcome message without actual grant
    if (pfid.startsWith('local_')) {
      console.log('[playfab-welcome] Local session, returning simulated welcome bonus');
      return jsonResponse(200, {
        ok: true,
        granted: WELCOME_BONUS,
        message: `Welcome! You have ${WELCOME_BONUS.PT} PT and ${WELCOME_BONUS.GC} GC to get started.`,
        local: true
      });
    }
    ...
    try {
      ...
      if (!ptResult.ok || !gcResult.ok) {
        ...
        return jsonResponse(200, { ok: true, granted: WELCOME_BONUS, fallback: true });
      }
      ...
      console.warn('[playfab-welcome] PlayFab error:', playfabError.message);
      return jsonResponse(200, {
        ok: true,
        granted: WELCOME_BONUS,
        message: `Welcome! You have ${WELCOME_BONUS.PT} PT and ${WELCOME_BONUS.GC} GC to get started.`,
